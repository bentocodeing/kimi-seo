...
1. Check credentials: `kimi-seo run backlinks_auth.py --check --json`
...
5. Format output to match kimi-seo conventions

## Tier-Based Workflow

### Tier 0 (Always Available, No Config Needed)
- Common Crawl domain metrics: `kimi-seo run commoncrawl_graph.py <domain> --json`
  - PageRank, PageRank rank, harmonic centrality, harmonic centrality rank, crawl/ranking presence
- If known backlinks provided, verify them: `kimi-seo run verify_backlinks.py --target <url> --links <file> --json`
...
### Tier 1 (+ Moz API)
- All Tier 0 checks
- Moz URL metrics: `kimi-seo run moz_api.py metrics <url> --json`
  - DA, PA, Spam Score, link counts, referring domains
- Moz referring domains: `kimi-seo run moz_api.py domains <url> --json`
- Moz anchor text: `kimi-seo run moz_api.py anchors <url> --json`
- Moz top pages: `kimi-seo run moz_api.py pages <domain> --json`
...
- All Tier 1 checks
- Bing inbound links: `kimi-seo run bing_webmaster.py links <url> --json`
- For comparison between two properties registered to the same Bing account:
  `kimi-seo run bing_webmaster.py compare <url1> <url2> --json`
...
```bash
kimi-seo run validate_backlink_report.py --report report_data.json --json
```
