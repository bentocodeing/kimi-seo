...
2. Use `kimi-seo run render_page.py <URL> --mode auto --json` before HTML/source inspection so SPA content is visible when needed
...
kimi-seo run pagespeed_check.py URL --json

# SPA-aware HTML/render inspection
kimi-seo run render_page.py URL --mode auto --json
...
```bash
kimi-seo run pagespeed_check.py URL --json
kimi-seo run crux_history.py URL --json
```
