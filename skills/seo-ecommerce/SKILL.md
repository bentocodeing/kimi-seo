...
```
1. kimi-seo run render_page.py <url> --mode auto → raw/rendered HTML
2. kimi-seo run parse_html.py --url <url>   → SEO elements
...
```bash
kimi-seo run dataforseo_costs.py check merchant_google_products_search
```
...
After each call:
```bash
kimi-seo run dataforseo_costs.py log merchant_google_products_search <cost>
```
...
kimi-seo run dataforseo_merchant.py search "<keyword>" --marketplace google

# Seller analysis: merchant ratings and dominance
kimi-seo run dataforseo_merchant.py sellers "<keyword>"

# Normalize results for analysis
kimi-seo run dataforseo_normalize.py results.json --module merchant
```
...
```bash
kimi-seo run dataforseo_costs.py check merchant_amazon_products_search
```
...
kimi-seo run dataforseo_merchant.py search "<keyword>" --marketplace amazon

# Cross-marketplace comparison
kimi-seo run dataforseo_merchant.py compare "<keyword>"
```
...
kimi-seo run ucp_check.py https://store.example.com --json

# With endpoint reachability probes (HEAD each declared capability)
kimi-seo run ucp_check.py https://store.example.com --probe-endpoints --json
```
