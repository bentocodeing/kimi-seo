...
**Runtime context:** Load `references/flow-framework.md` on every `/kimi-seo:seo flow` activation.
...
|---------|-------------|
| `/kimi-seo:seo flow` | Show FLOW overview + stage menu |
| `/kimi-seo:seo flow find [url\|topic]` | Find-stage: keyword research, gap analysis, SERP intent mapping (5 prompts) |
| `/kimi-seo:seo flow leverage [url]` | Leverage-stage: backlink strategy, off-site authority (1 prompt) |
| `/kimi-seo:seo flow optimize [url]` | Optimize-stage: select 2-3 most relevant of 21 prompts based on context |
| `/kimi-seo:seo flow win [url]` | Win-stage: BOFU, conversion rate, dual-surface scorecard (3 prompts) |
| `/kimi-seo:seo flow local [url]` | Local-stage: GBP optimization, meta, title tags, local audits (11 prompts) |
| `/kimi-seo:seo flow prompts` | Full index of all 41 prompts (stage, name, trigger conditions) |
| `/kimi-seo:seo flow sync` | Pull latest prompt files from github.com/AgriciDaniel/flow |

---

## Orchestration Logic

### On `/kimi-seo:seo flow` (no sub-command)
...
### On `/kimi-seo:seo flow find [url|topic]`
1. Read all files in `references/prompts/find/`
2. Apply each prompt to the URL or topic
3. Cross-reference: "For deeper SERP clustering, see `/kimi-seo:seo cluster <seed-keyword>`"

### On `/kimi-seo:seo flow leverage [url]`
1. Read the file in `references/prompts/leverage/`
2. Apply to the URL's current backlink context
3. Cross-reference: "For raw backlink data, see `/kimi-seo:seo backlinks <url>`"

### On `/kimi-seo:seo flow optimize [url]`
...
4. Apply selected prompts; note the others are accessible via `/kimi-seo:seo flow prompts`
5. Cross-reference: "For full content quality analysis, see `/kimi-seo:seo content <url>` and `/kimi-seo:seo geo <url>`"

### On `/kimi-seo:seo flow win [url]`
1. Read all files in `references/prompts/win/`
2. Apply each prompt to the URL's conversion and BOFU context
3. Cross-reference: "For SXO persona scoring, see `/kimi-seo:seo sxo <url>`"

### On `/kimi-seo:seo flow local [url]`
1. Read all files in `references/prompts/local/`
2. Apply to the URL's local SEO context
3. Cross-reference: "For full local SEO analysis, see `/kimi-seo:seo local <url>` and `/kimi-seo:seo maps [command]`"

### On `/kimi-seo:seo flow prompts`
1. Read `references/prompts/README.md`
2. Display the full index: all 41 prompts with stage, name, trigger conditions

### On `/kimi-seo:seo flow sync`
1. Run: `kimi-seo run sync_flow.py`
...
- `references/flow-framework.md`: FLOW operating model (load on every `/kimi-seo:seo flow` activation)
- `references/bibliography.md`: Evidence sources; load when citing studies or statistics
- `references/prompts/README.md`: Prompt index; load for `/kimi-seo:seo flow prompts`
- `references/prompts/find/`: 5 prompts; load for `/kimi-seo:seo flow find`
- `references/prompts/leverage/`: 1 prompt; load for `/kimi-seo:seo flow leverage`
- `references/prompts/optimize/`: 21 prompts; load selectively for `/kimi-seo:seo flow optimize`
- `references/prompts/win/`: 3 prompts; load for `/kimi-seo:seo flow win`
- `references/prompts/local/`: 11 prompts; load for `/kimi-seo:seo flow local`

---

## Attribution

Every `/kimi-seo:seo flow` activation (any sub-command) outputs before analysis:
...
| Scenario | Action |
|----------|--------|
| `references/flow-framework.md` missing | "FLOW reference files not synced. Run: `/kimi-seo:seo flow sync`" |
| Prompt file missing | "Run `/kimi-seo:seo flow sync` to pull the latest prompts from the FLOW repo." |
