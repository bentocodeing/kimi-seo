...
# DataForSEO Extension Installer for Kimi SEO
...
main() {
    ...
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        _GLOB_MATCH=$(ls -d "${HOME}/.claude/plugins/cache/*/kimi-seo/"*/skills/seo 2>/dev/null | tail -n1 || true)
    ...
    if [ -f "${SCRIPT_DIR}/skills/seo-dataforseo/SKILL.md" ]; then
        ...
        echo "  Run this script from the kimi-seo repo: ./extensions/dataforseo/install.sh"
        exit 1
    fi
...
PY
    ...
    echo "  2. Run commands:"
    echo "     /kimi-seo:seo dataforseo serp best coffee shops"
    echo "     /kimi-seo:seo dataforseo keywords seo tools"
    echo "     /kimi-seo:seo dataforseo backlinks example.com"
    echo "     /kimi-seo:seo dataforseo ai-mentions your brand"
    echo ""
